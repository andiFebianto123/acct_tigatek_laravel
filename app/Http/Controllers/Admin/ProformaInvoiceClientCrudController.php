...
class ProformaInvoiceClientCrudController extends CrudController
{
    ...
    public function edit($id)
    {
        ...
        if ($entry->proforma_invoice_client_details) {
            foreach ($entry->proforma_invoice_client_details as $d) {
                $d->reason = $d->reason ?? '';
            }
        }
    ...
    public function printInvoice($id)
    {
        ...
        if ($attachmentPath) {
            try {
                ...
                // 1. Render proforma invoice HTML directly into mPDF (no FPDI parsing needed for proforma invoice)
                $html = view('exports.invoice-client-proforma-single-pdf', $data)->render();
                $mpdf->WriteHTML($html);
                ...
                Log::error('Proforma PDF merge attachment failed: ' . $e->getMessage());
