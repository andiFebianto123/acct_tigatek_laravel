...
class CastAccountsLoanCrudController extends CrudController
{
    ...
    public function update()
    {
        ...
        if (request()->has('type') && request()->type == 'cast_account') {
            $id = request('id') ?? request()->route('id') ?? '';
            CRUD::setValidation($this->ruleValidationCastAccountDetails($id));
            ...
            DB::beginTransaction();
            try {
                $item = $this->service->updateCastAccountLoanDetails((int)$request->id, $dto);
                ...
                DB::rollBack();
                Log::error('Error updating cast account loan details: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
                return response()->json([
                    'status' => false,
                    'success' => false,
                    'error' => $e->getMessage(),
