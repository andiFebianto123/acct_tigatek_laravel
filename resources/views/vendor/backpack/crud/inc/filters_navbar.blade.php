<nav class="navbar navbar-expand-lg {{$table_name."-filters" ?? ''}} navbar-filters mb-0 py-0 shadow-none">
      ...
      <div class="collapse navbar-collapse" id="bp-filters-navbar">
        <ul class="nav navbar-nav">
          ...
          @if (isset($filter_table))
            @foreach ($filter_table as $filter)
                @includeFirst($filter->getNamespacedViewWithFallbacks())
            @endforeach
            <li class="nav-item"><a href="#" id="remove_filters_button_{{$table_name}}" class="nav-link {{ count(Request::input()) != 0 ? '' : 'invisible' }}"><i class="la la-eraser"></i> {{ trans('backpack::crud.remove_filters') }}</a></li>
          @else
...
@push('crud_list_scripts')
  ...
  @if (isset($filter_table))
    ...
    <script>
      ...
      var refreshDatatablesOnFilterChange = function(url, name_table)
      {
          SIAOPS.getAttribute(name_table).table.ajax.url(url).load();
      }

      var updateDatatablesOnFilterChange = function(filterName, filterValue, update_url = false, debounce = 500) {
      ...
      var callFunctionOnce = function(func, within = 300, timerId = null) {
      ...
      var addOrUpdateUriParameter = function(uri, parameter, value, name_table) {
          ...
          var datatable_instance = SIAOPS.getAttribute(name_table);
          
          if(new_url.toString() == datatable_instance.route){
              $("#remove_filters_button_"+name_table).toggleClass('invisible', true);
          }else{
              $("#remove_filters_button_"+name_table).toggleClass('invisible', !new_url.query());
          }
