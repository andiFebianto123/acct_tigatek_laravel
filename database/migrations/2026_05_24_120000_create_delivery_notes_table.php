    ...
    public function up(): void
    {
        if (!Schema::hasTable('delivery_notes')) {
            Schema::create('delivery_notes', function (Blueprint $table) {
                ...
                $table->unsignedBigInteger('client_id')->nullable()->comment('ID Client (Kirim Ke)');
                $table->text('address')->nullable()->comment('Alamat pengiriman / Kirim Ke');
                $table->date('date')->comment('Tanggal Surat Jalan');
                $table->string('number', 50)->comment('Nomor Surat Jalan');
